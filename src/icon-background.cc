...
IconBackground::Vertex::Vertex(float x, float y, float z, float u, float v)
    : x(x), y(y), z(z), u(u), v(v)
{}
...
bool
IconBackground::Init(Cuboid& cuboid)
{
  if (!initialized_ && Setup() == false) {
    return false;
  }
...
void
IconBackground::ConstructVertices()
{
...
void
IconBackground::ConstructElements()
{
